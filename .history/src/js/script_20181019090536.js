(function () {
	...
	class Todo {
		...
		deleteTask(e) {
			...
			this.todoArr = this.todoArr.filter((obj) =>{
				console.log(obj.id + ' ' + item.id);
				obj.id !== item.id;
			});
			
			console.log('deleteTask arrayA ',this.todoArr);
	...
	class Task {
		...
		getTime() {
			const data = new Date();
			return `${data.getDate()}-${data.getMonth()+1}-${data.getFullYear()} || ${data.getHours()}:${data.getMinutes().padStart(1, '0' )} `
		}
